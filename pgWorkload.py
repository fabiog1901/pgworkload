...
def main():
    ...
    for _ in range(args.concurrency):
        mp.Process(target=worker, args=(
            q, kill_q, args.dburl, args.parameters, args.iterations, args.duration, args.conn_duration)).start()
...
def signal_handler(sig, frame):
    ...
    logging.info("KeyboardInterrupt signal detected. Printing final stats.")
    stats.print_stats()
    sys.exit(0)
...
def run_transaction(conn, op, max_retries=3):
    for retry in range(1, max_retries + 1):
        try:
            ...
            if str(e) != 'sending query failed: another command is already in progress':
...
def worker(q, kill_q, dburl: str, parameters: list, iterations: int, duration: int, conn_duration: int):
